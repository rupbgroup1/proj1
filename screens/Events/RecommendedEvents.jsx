...
export default class RecommendedEvents extends React.Component {
  ...
  getUser() {
    AsyncStorage.getItem('user', (err, userJSON) => {
      ...
      this.setState({ user: userObj }, () =>
        this.fetchGetEvents(userObj.UserId, userObj.NeighborhoodName));
    }
    );
  ...
  fetchGetEvents(userId, nei) {
    console.log("in fetch");
    return fetch('http://proj.ruppin.ac.il/bgroup29/prod/api/Events/Recommended/' + userId+'/'+nei, {
      ...
      .then(
        (result) => {
          if (result.length > 0) {
            ...
            Alert.alert(" לא נמצאו אירועים מומלצים");
        },
        (error) => {
